...
// _pos:centra point position; _axes_rev:self-rotate axes; _size:size of the sphere. _rev_speed: self-rotate speed; _rota_speed: rotate sp
...
// Set the rotate direction as random data
      m_axes((float)rand() / (float)RAND_MAX, (float)rand() / (float)RAND_MAX, (float)rand() / (float)RAND_MAX)
{
	// self-rotate direction get normalized
	m_axes.normalize();

	// If no texture then we return
	if (! texname )
	return;

	// 类的构建
	Texture texture;
        texture.Init("extra/earth.bmp",1);//初始化成为一个opengl的文件
	
	m_tex = texture.mTextureID;
...
{
	// Compute the angle 'dtheta' and 'drho'
...
void planet::draw()
{
	...
	if (m_sun)
	{
		// Then close the sun light, and texture.
